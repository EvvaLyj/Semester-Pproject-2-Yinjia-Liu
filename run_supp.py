...
parser.add_argument('-aug', '--aug-type', default='default', type=str,
                    help='data augmentation type', choices=['default', 'none', 
                                                            ...
                                                            'amp_gm_v5', 
                                                            'amp_GED',
                                                            'phs_TI_square','phs_TI_square_control','phs_TI_no_noise_control',
                                                            'phs_hsv_TI_no_noise_control',
                                                            'phs_sig','phs_sig_control',
                                                            'amp_gm_v1_phs_sig',
                                                            'amp_gm_v1_phs_TI_square','amp_gm_v1_phs_TI_square_control',
...
def main():
    ...
    train_loader_simclr, train_loader_fftclr = dataset_train_fft(args)

    model = ResNetSimCLR(base_model=args.arch, out_dim=args.out_dim)

    optimizer = torch.optim.Adam(model.parameters(), args.lr, weight_decay=args.weight_decay)

    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=len(train_loader_simclr), eta_min=0,
                                                           last_epoch=-1)
    ...
    with torch.cuda.device(args.gpu_index):
        ...
        simclr.train_fft_load(train_loader_simclr, train_loader_fftclr, checkpoint=ckp)
